...
class TFIDF(object):
    ...
    def search(self, question):       
        ...
        results = [(self.documents[item[0]][0], item[1], item[0]) for item in sorted(ranking.items(), key=lambda t: t[1], reverse = True) if item[1] > 0]        
        return results
    ...
    def doc_question_similarity(self, doc_index, question_tfidfs):
        return Vector.similarity(self.tfidf_by_doc_index(doc_index), question_tfidfs)
